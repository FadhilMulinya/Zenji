...
import { ensureUser } from "../user/user.controller";
import { getBalances, receiveFaucet } from "../bank/bank.controller";
import { createNewWallet, importWalletFromMnemonic, importWalletFromPrivateKey } from "../wallet/wallet.controller";
...
export const handleText = async (ctx: MyContext) => {
  ...
  if (!state) {
    await ctx.reply(`I received your message: "${message}". Type /help to see what I can do.`);
    return;
  }
