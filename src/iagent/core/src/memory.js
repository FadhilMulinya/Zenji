...
class MemoryManager {
    ...
    async addEmbeddingToMemory(memory) {
        ...
        try {
            ...
            memory.embedding = await (0, embedding_ts_1.embed)(this.runtime, memoryText);
        }
        catch (error) {
            logger_ts_1.default.error("Failed to generate embedding:", error);
            // Fallback to zero vector if embedding fails
            memory.embedding = (0, embedding_ts_1.getEmbeddingZeroVector)().slice();
        }
        return memory;
    }
    /**
     ...
     */
    async getMemories({ roomId, count = 10, unique = true, start, end, }) {
    ...
    async searchMemoriesByEmbedding(embedding, opts) {
        const { match_threshold = defaultMatchThreshold, count = defaultMatchCount, roomId, unique, } = opts;
    ...
    async createMemory(memory, unique = false) {
        ...
        if (existingMessage) {
            logger_ts_1.default.debug("Memory already exists, skipping");
            return;
        }
        logger_ts_1.default.log("Creating Memory", memory.id, memory.content.text);
