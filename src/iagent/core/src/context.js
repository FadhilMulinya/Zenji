...
exports.composeRandomUser = exports.addHeader = exports.composeContext = void 0;
 ...
 * const contextSimple = composeContext({ state, template });
 */
const composeContext = ({ state, template, templatingEngine, }) => {
    const templateStr = typeof template === "function" ? template({ state }) : template;
    if (templatingEngine === "handlebars") {
        const templateFunction = handlebars_1.default.compile(templateStr);
 ...
 */
const composeRandomUser = (template, length) => {
    const exampleNames = Array.from({ length }, () => (0, unique_names_generator_1.uniqueNamesGenerator)({ dictionaries: [unique_names_generator_1.names] }));
    let result = template;
...
exports.composeRandomUser = composeRandomUser;
