...
import type * as GovernanceTypes from "../types/gov.ts";
import {
...
export async function getProposal(
	...
	try {
		const result = await this.chainGrpcGovApi.fetchProposal(params.proposalId);
		return createSuccessResponse(result);
	} catch (err) {
...
export async function msgSubmitProposalSpotMarketLaunch(
	...
	try {
		...
		return createErrorResponse("msgSubmitProposalSpotMarketLaunchError", err);
	}
}
